...
from PyQt6.QtWidgets import QWidget, QMainWindow, QApplication, QMdiArea, QMdiSubWindow, QToolBar, QVBoxLayout, \
    ...
    QCheckBox, QSizePolicy, QDialog, QFormLayout, QDialogButtonBox, QCalendarWidget, QTimeEdit
...
class DateTimeSelectionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        form_layout.addRow("Выберите время:", self.time_edit)
...
class RecordsViev_subwindow(QMdiSubWindow):
    ...
    def on_rtt_result_message(self, result):
        ...
        first_records = str(pdb.datetime_from_timestamp(first_records))[:-4]
        last_records = str(pdb.datetime_from_timestamp(last_records))[:-4]
...
class SignalsView_subwindow(QMdiSubWindow):
    def __init__(self, parent=None):
        ...
        self.timeEdit = ResizableLineEdit("0000-00-00 00:00:00.0")
        self.timeEdit.setToolTip("Дата и время записи (YYYY-MM-DD HH:MM:SS.s)")
        self.timeEdit.setPlaceholderText("YYYY-MM-DD HH:MM:SS.s")
        ...
        self.timeEdit.textEdited.connect(self.time_edited)

        self.selectTimeButton = QToolButton()
        self.selectTimeButton.setText("📅")
        self.selectTimeButton.setToolTip("Выбрать дату и время")
        self.selectTimeButton.setMinimumWidth(20)
        self.selectTimeButton.setAutoRaise(True)
        self.navi_toolbar.addWidget(self.selectTimeButton)
        self.selectTimeButton.clicked.connect(self.show_datetime_dialog)
    ...
    def save_points_to_file(self):
        """
        Сохраняет данные points текущей записи в бинарный файл.
        """
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "Успех",
                f"Данные points сохранены в файл: {filename}"
            )
    ...
    def time_edited(self):
        time_text = self.timeEdit.text().strip()
        try:
            dt = datetime.datetime.strptime(time_text, "%Y-%m-%d %H:%M:%S.%f")
            timestamp = int(dt.timestamp() * 1000)
        except ValueError:
            self.on_error_message("Некорректный формат времени. Используйте YYYY-MM-DD HH:MM:SS.s")
            return
    ...
    def show_datetime_dialog(self):
        ...
        """
        dialog = DateTimeSelectionDialog(self)
        ...
        if dialog.exec():
            ...
            self.set_current_rec_num(closest_index)
            self.timeEdit.blockSignals(True)
            self.timeEdit.setText(selected_dt.toString("yyyy-MM-dd HH:mm:ss.z"))
            self.timeEdit.blockSignals(False)
            self.select_and_plot_record(closest_index)
            QMessageBox.information(
                self,
                "Выбор сделан",
                f"Загрузка осциллограммы с {selected_dt.toString('dd.MM.yyyy HH:mm:ss')}"
            )
