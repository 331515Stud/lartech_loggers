...
from PyQt6.QtGui import QIcon, QCursor, QColor, QImage
...
class TrendsSubwindow(QMdiSubWindow):
    def __init__(self, parent=None):
        ...
        # Кнопки
        def set_button_icon(button, icon_path, active_icon_path=None):
            ...
            if active_icon_path and os.path.exists(active_icon_path):
                button.active_icon_path = active_icon_path
            else:
                button.active_icon_path = None
        ...
        set_button_icon(self.go_to_cursor_button, "./icons/center_cursor.png")
        ...
        self.toolbar_layout.addWidget(self.go_to_cursor_button)

        self.center_cursor_button = QToolButton()
        set_button_icon(self.center_cursor_button, "./icons/custom_center_view.png")
        ...
        self.toolbar_layout.addWidget(self.center_cursor_button)

        self.send_timestamp_button = QToolButton()
        set_button_icon(self.send_timestamp_button, "./icons/send_to_signals.png")
        ...
        self.toolbar_layout.addWidget(self.send_timestamp_button)

        self.lock_cursor_button = QToolButton()
        set_button_icon(self.lock_cursor_button, "./icons/lock_cursor_active.png", "./icons/lock_cursor.png")
        ...
        self.toolbar_layout.addWidget(self.lock_cursor_button)
    ...
    def toggle_lock_cursor_mode(self):
        ...
        if self.lock_cursor_mode:
            ...
            # При закреплении — закрытый замок
            self.lock_cursor_button.setIcon(QIcon("./icons/lock_cursor.png"))
        else:
            # При откреплении — открытый замок
            self.lock_cursor_button.setIcon(QIcon("./icons/lock_cursor_active.png"))
        self.update_values()

    def update_cursor_on_range_change(self, viewbox, range):
        if self.lock_cursor_mode:
            x_range = range[0]  # Получаем текущий видимый диапазон по X
    ...
    def update_values(self):
        if not self.all_data.empty and self.all_time_labels and self.all_valid_indices:
            ...
            if 0 <= idx < len(self.all_valid_indices):
                ...
                display_text = f"<span style='color:#ffffff; font-weight:700;'>Date/Time:</span> {date_time}<br><br>"
                for col in self.columns:
                    try:
                        value = self.all_data[col].iloc[data_idx]
                        val_str = f"{value:.2f}" if pd.notna(value) else "N/A"
                        display_text += f"<span style='color:#ffffff;'>{col}:</span> <span style='color:white;'>{val_str}</span><br>"
                    except:
                        display_text += f"<span style='color:#ffffff;'>{col}:</span> <span style='color:white;'>N/A</span><br>"
    ...
    def on_rms_calculated(self, df, chunk_index, total_chunks):
        ...
        if self.processed_chunks < total_chunks:
            self.status_label.setText(f"Pагрузка данных...")
        else:
            self.status_label.setText(f"Lанные загружены")
