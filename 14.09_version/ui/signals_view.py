...
class SignalsView_subwindow(QMdiSubWindow):
    ...
    def __init__(self, parent=None):
        ...
        for plot_row in range(7):
            ...
            for plot_col in range(2):
                ...
                plot.getAxis('left').setTextPen('w')  # White text for Y-axis
                plot.getAxis('bottom').setTextPen('w')  # White text for X-axis
                plot.setLabel('bottom', 'Time' if plot_col == 0 else 'Frequency', color='white')
                plot.setLabel('left', f'Channel {plot_row + 1}' if plot_col == 0 else 'Amplitude', color='white')
                plot.showGrid(x=True if plot_col == 1 else False, y=True, alpha=0.3)  # Grid with same alpha as TrendsSubwindow
    ...
    def on_next_rec_result(self, rec_dict):
        ...
        # Automatically send the new timestamp to TrendsSubwindow
        ...
        for i in range(len(self.channel_boolmask)):
            ...
            if self.channel_boolmask[i] and self.plot_array[i][0] is None:
                ...
                for col in range(2):
                    ...
                    plot.setLabel('bottom', 'Time' if col == 0 else 'Frequency', color='white')
                    plot.setLabel('left', f'Channel {i + 1}' if col == 0 else 'Amplitude', color='white')
