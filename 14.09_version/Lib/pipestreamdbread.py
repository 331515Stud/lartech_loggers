...
db_connection_params = {"user":"postgres", "password":"nikita87993", "host":"localhost",
                       "port":"5432", "database":"streampipe"}
...
def get_record(cursor, tablename:str, timestamp, colnames_list):
    ...
    return record_dict
#-----------------------------------------------------------------------------------------------------
def get_rms_trend(cursor, tablename: str):
    #возвращает словарь со списками, содержащими пересчитанные в вольты и амперы значениями
    rms_colnames = ["timestamp", "add_data_0", "add_data_1", "add_data_2", "add_data_3", "add_data_4", "add_data_5"]
    rms_colnames_str = ", ".join(rms_colnames)

    multypliers_tab_colnames = ["cfg_voltage_multiplier", "cfg_voltage_divider", "cfg_current_multiplier", "cfg_current_divider"]
    multypliers_names = ["VoltMult", "VoltDiv", "CurrMult", "CurrDiv"]
    multypliers_colnames_str = ", ".join(multypliers_tab_colnames)

    query_last = f'''SELECT {multypliers_colnames_str} FROM {tablename} 
            ORDER BY timestamp DESC
            LIMIT 1; '''
    cursor.execute(query_last)
    last_ans = cursor.fetchone()
    ...
    print(f"Multypliers: {mult_dict}")

    query = f'''SELECT {rms_colnames_str} FROM {tablename}'''
    cursor.execute(query)
    record_tuple_list =  cursor.fetchall()
    val_names = ["timestamp", "U_A_rms", "U_B_rms", "U_C_rms", "I_A_rms", "I_B_rms", "I_C_rms"]
    rms_raw_dict = dict(zip(val_names, zip(*record_tuple_list)))
    ...
    timestamps = list(rms_raw_dict["timestamp"])
    U_A_rms = list(map(get_voltage, rms_raw_dict["U_A_rms"])) 
    U_B_rms = list(map(get_voltage, rms_raw_dict["U_B_rms"])) 
    U_C_rms = list(map(get_voltage, rms_raw_dict["U_C_rms"])) 

    I_A_rms = list(map(get_ampertage, rms_raw_dict["I_A_rms"])) 
    I_B_rms = list(map(get_ampertage, rms_raw_dict["I_B_rms"])) 
    I_C_rms = list(map(get_ampertage, rms_raw_dict["I_C_rms"]))

    rms_dict = dict(zip(val_names, [timestamps, U_A_rms, U_B_rms, U_C_rms, I_A_rms, I_B_rms, I_C_rms]))
